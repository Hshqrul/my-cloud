...
<div class="isolate flex flex-col">
    ...
    <flux:separator variant="sublte" class="mt-5 mb-5" />
    <div class="mt-5">
        <flux:callout icon="sparkles" icon-variant="outline" color="{{ $rsvp->attendence ? 'indigo' : 'gray' }}" inline>
            <flux:callout.heading>
                {{ __('Thanks for your response!') }}
            </flux:callout.heading>

            <flux:callout.text>
                @if ($rsvp->attendence == true)
                    {{ __('We are so excited to see you') }}, {{ $rsvp->no_of_pax > 1 ? 'both of you' : $rsvp->name }}!
                    👋
                @else
                    {{ __('We really hope that you are coming.') }} ☹️
                @endif
            </flux:callout.text>
            <x-slot name="actions">
                <flux:tooltip content="Buy me a coffee">
                    <flux:button href="https://buymeacoffee.com/hashaqirul">
                        <svg width="24" height="24" viewBox="0 0 884 1279" fill="currentColor"
    ...
    <div class="flex flex-col justify-between py-2 px-3 mt-5">
